...
def registration(fixed_image_path, moving_image_path, dilated_segmentation_path, initial_transform_path):
    """
    ...
    moving_image_mask = ants.image_read(dilated_segmentation_path)
    ...
    registration = ants.registration(fixed=fixed_image, moving=moving_image,
                                  ...
                                  convergence_threshold=[1e-8, 1e-8],
                                  sampling_percentage=0.25,
                                  transform_type='Rigid',
                                  initial_transform=initial_transform_path)
    transformation = registration['fwdtransforms'][0]
...
def register_volumes(dynact_dir, output_segmentation_dir, output_transformation_dir, wbct_segmentation_path, frame_start, frame_stop, tolerance, bone, model, motion, logger):
    ...
    frame_1_dynact_masked_and_cropped = sitk.Mask(frame_1_dynact_cropped, wbct_mask) # apply mask on dynamic ct frame 1

    # calcuating intensity and tolerance
    start_intensity = ((sitk.GetArrayFromImage(frame_1_dynact_masked_and_cropped)[(sitk.GetArrayFromImage(wbct_mask) > 0) & (np.absolute(sitk.GetArrayFromImage(frame_1_dynact_masked_and_cropped)) > 0)])).mean()
    ...
    dilated_segmentation_dir = os.path.join(output_segmentation_dir, f"{bone}_dilated_segmentation.nii")
    sitk.WriteImage(wbct_segmentation_dilated, dilated_segmentation_dir)
    ...
    while index < len(frames):
        ...
        initial_transformation_dir = os.path.join(output_transformation_dir, f"VOLUME_1_TO_{item}_{bone}_INIT_TFM.mat")
        sitk.WriteTransform(hand_segmentation_transformation, initial_transformation_dir)
        
        # running registration
        final_tmat = registration(
            fixed_image_path=current_dynact_path, 
            moving_image_path=frame_1_dynact_path, 
            dilated_segmentation_path=dilated_segmentation_dir,
            initial_transform_path=initial_transformation_dir
            )
        
        fixed_image_ants = ants.image_read(current_dynact_path)
        wbct_segmentation_ants = ants.image_read(wbct_segmentation_path)

        final_mask = ants.apply_transforms(fixed=fixed_image_ants, moving=wbct_segmentation_ants, transformlist=final_tmat)

        # writing transform and transformed segmentation
        try:
            ants.image_write(final_mask, os.path.join(output_segmentation_dir, f"VOLUME_1_TO_{item}_{bone}_MASK_REG.nii"))
        except Exception as e:
            print(f"ERROR: {e}")

        # try:
        #     ants.write_transform(final_tmat, os.path.join(output_transformation_dir, f"VOLUME_1_TO_{item}_{bone}_REG.tfm"))
        # except Exception as e:
        #     print(f"ERROR: {e}")


        # reload in final mask as sitk 
        final_mask = sitk.ReadImage(os.path.join(output_segmentation_dir, f"VOLUME_1_TO_{item}_{bone}_MASK_REG.nii"), sitk.sitkUInt8)
        ...
        logger.warning(f"{model}, {motion}, {bone}, {item}, {result}, {start_intensity}, {new_intensity}")
        os.remove(initial_transformation_dir)

    # while loop end

    os.remove(dilated_segmentation_dir)        
...
def main(models_dir, model, motion, frame_start, frame_stop, bone):
    ...
    # Logger setup
    logger_filename = os.path.join(models_dir, f"{model}_logs_ants.log")
    ...
    logger.warning(f"model, motion, bone, frame, result, start_intensity, new_intensity")
