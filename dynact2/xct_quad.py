...
def main():
    ...
    resampled_js_img = sitk.BinaryDilate(resampled_js_img, (9,9,9))
    ...
    direction_mat = shape_stats2.GetOrientedBoundingBoxDirection(1)
    ...
    resampler.SetOutputOrigin(shape_stats2.GetOrientedBoundingBoxOrigin(1))
    ...
    # Change the image axes order so that we have a nice display (only for TRP)
    if "trp" in xct_seg_path.lower():
        obb_img = sitk.PermuteAxes(obb_img, [2, 1, 0])
    
    sitk.WriteImage(obb_img, os.path.splitext(xct_seg_path)[0] + "_ROTATED.nrrd")
    ...
    # resampled_js_img = sitk.Resample(xct_js_img, bb_xct_img, interpolator=sitk.sitkNearestNeighbor)
    ...
    if slice_axis:
        # dilated_js = sitk.BinaryDilate(bb_js_img, (0, 0, 2*int(bb_xct_img.GetSize()[2])+1))
        dilated_js = sitk.BinaryDilate(bb_js_img, (2*int(bb_xct_img.GetSize()[2])+1, 0, 0))
    else:
        dilated_js = sitk.BinaryDilate(bb_js_img, (0, 2*int(bb_xct_img.GetSize()[2])+1, 0))

    dilated_js = sitk.Resample(dilated_js, bb_xct_img, interpolator=sitk.sitkNearestNeighbor)
    sitk.WriteImage(dilated_js, os.path.splitext(xct_seg_path)[0] + "_JS_BB_DILATE.nii")
    
    masked_bone = sitk.Mask(bb_xct_img, dilated_js)
    ...
    if "mc1" in xct_seg_path.lower():
        ...
        bb_masked_bone = bb_masked_bone[size_z-mc1_length:, :, :]
        bb_masked_bone = sitk.Resample(bb_masked_bone, bb_xct_img, interpolator=sitk.sitkNearestNeighbor)
        bb_masked_bone = bounding_box(masked_bone, min_val, min_val)
